            ...
            (async () => {
              ...
              try {
                if (spaceId) {
                  ...
                  const user = await component.$matrixService.getUserByMatrixId(participant.id, room);
                  const filters = user?.profile?.settings;
                  const data = await component.$userService.getUsersByAdvancedFilter(
                      filters, 0, 1, '', 'all', cleanQuery, false, null, 'true'
                  );

                  const matched = data?.users?.[0];
                  const isMatch = matched?.username === user?.profile?.username;

                  const results = isMatch ? [{
                    uid: matched.username,
