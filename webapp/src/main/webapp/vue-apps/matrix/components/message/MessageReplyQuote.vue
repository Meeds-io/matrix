...
<template>
  <v-card
    class="white pa-2 border-radius-16 clickable"
    flat
...
export default {
  ...
  computed: {
    ...
    thumbnailWidth() {
      return this.targetMessage?.content?.info?.w || this.targetMessage.content?.w;
    },
    thumbnailHeight() {
      return this.targetMessage?.content?.info?.h || this.targetMessage.content?.h;
    },
    thumbnailURL() {
      return this.targetMessage?.content?.thumbnail_url;
    },
    url() {
      return this.targetMessage?.content?.url;
    },
    thumbnailAspectRatio() {
      return this.thumbnailWidth / this.thumbnailHeight;
    },
    imageThumbnailURL() {
