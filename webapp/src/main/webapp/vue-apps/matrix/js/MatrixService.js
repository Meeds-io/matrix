  ...
  for (const item of messageItems) {
    ...
    if (relatesTo?.rel_type === 'm.replace' && newContent) {
      ...
      if (originalMessage && !isRedacted(originalMessage)) {
        const applied = applyEditToMessage(originalMessage, newContent, item, lastAppliedEditTsMap, targetEventId, formatPromises, room);
        if (!applied) {
          continue;
        }
...
function applyEditToMessage(originalMessage, newContent, item, lastAppliedEditTsMap, targetEventId, formatPromises, room) {
