...
export function buildReplyToObject(messages, eventId) {
  ...
  while (ancestor.content?.['m.relates_to']?.['m.in_reply_to']?.event_id) {
    const ancestorEventId = ancestor.content['m.relates_to']['m.in_reply_to'].event_id;

    if (visited.has(ancestorEventId)) {
      break;
    }
    visited.add(ancestorEventId);

    const ancestorEvent = getMessageById(ancestorEventId);
    if (!ancestorEvent) {
      break;
    }

    targetUser = ancestorEvent.sender;
    targetEventId = ancestorEvent.event_id;
    ancestor = ancestorEvent;
    targetType = ancestorEvent.content?.msgtype;
    targetThumbnailURL = ancestor.content?.thumbnail_url;
    TargetUrl = ancestor.content?.url;
    targetThumbnailWidth = ancestor.content?.info?.w || ancestor.content?.w;
    targetThumbnailHeight = ancestor.content?.info?.h || ancestor.content?.h
    fileMimeType = ancestor.content?.info?.mimetype;
  }
